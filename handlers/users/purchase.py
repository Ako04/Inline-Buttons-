...
@dp.callback_query_handler(buy_callback.filter(item_name="ticket"))
async def buying_ticket(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=60)

    logging.info(f"{callback_data=}")
    quantity = callback_data.get("quantity")
    await call.message.answer(f"You chose Ticket. Good luck.",
                              reply_markup=buy_ticket)

@dp.callback_query_handler(buy_callback.filter(item_name="market"))
async def buying_market(call: CallbackQuery, callback_data: dict):
    await call.answer(cache_time=60)

    logging.info(f"{callback_data=}")
    quantity = callback_data.get("quantity")
    await call.message.answer(f"You chose Market. Good luck.",
